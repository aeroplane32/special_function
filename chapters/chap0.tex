...
\begin{itemize}
    \item \underline{原始观念}(primitive notion)，即不能被其他更基本的概念定义的概念，只有\underline{集合}(set)、\underline{类}(class)和\underline{属于}(“\(\in\)”)三个，其中“属于”是二元谓词，用来连接数学对象.
    \item 其他一切数学对象可以解释为集合或类，其他一切谓词都可解释为基本的逻辑连接词（或、且、非）以及“属于”.
\end{itemize}
...
\begin{definition}{函数}
    ...
    \begin{itemize}
        ...
        \item [\(\bullet\)] 对于定义域的子集\(X\)，\(f\upharpoonright X=\{\langle x,y \rangle|x \in X\}\)称为\(f\)的\uline{限制}(restriction)；在满足定义域外延性的前提下，\(f^{-1}=\{\langle y,x \rangle | \langle x,y \rangle \in f\}\)称为\(f\)的\uline{反函数}(inversion)；对于两个函数\(f,g\)，称\(g \circ f = \{\langle x,z \rangle| \exists y(y=f(x) \wedge z=g(x))\}\)称为\(f\)和\(g\)的\uline{复合函数}(composition).
    \end{itemize}
\end{definition}
...
\section*{前言：贯穿全书的一些符号约定}

\begin{itemize}
    \item “\(\Longrightarrow\)”是“于是”“所以”“可得”“推出”等的缩写；“\(\Longleftrightarrow\)”是“当且仅当”的缩写，全书都不使用“\(\because\)”“\(\therefore\)”这两个符号.
    \item “\(:=\)”是“定义为”的缩写.
    \item Kronecker \(\delta\)函数：\(\delta_{ij}:=\left\{\begin{aligned} & 1, & i=j \\ & 0, &i \neq j \end{aligned}\right.\)
    % \item Iverson括号：\(⁅P⁆ = \left\{\begin{aligned} & 1, & P\mbox{是真命题} \\ & 0, &P\mbox{是假命题} \end{aligned}\right.\)
    \item 示性函数：\(\chi_A(x) := \left\{\begin{aligned} & 1, & x \in A \\ & 0, &x \not \in A \end{aligned}\right.\) 
\end{itemize}
